...
// 🚀 التحديث: معمارية المستندات المستقلة لخفض التكلفة ومنع التضارب (NoSQL Optimized)
// ============================================================================
...
export const LiveStoreData = {
    cats: [], prods: [], settings: {}, banners: [],
    users: [], orders: [], deposits: [], payments: [], tiers: [], rates: {}, vault: [], coupons: [], offers: [], 
    alerts: [] 
};
...
export const DataManager = {
    ...
    updateUserProfile: function(updates) {
        ...
        // دمج التحديثات محلياً
        Object.assign(this.user, updates);
        
        // تحديث الرام
        const users = LiveStoreData.users || [];
        const idx = users.findIndex(u => Number(u.id) === Number(this.user.id));
        if (idx !== -1) users[idx] = this.user;
        
        // 🚀 إرسال التحديث إلى السحابة (تحديث مستند هذا المستخدم فقط لمنع التضارب)
        StoreDB.set(DB_KEYS.USERS, String(this.user.id), this.user)
            .catch(e => console.error("Cloud Sync Error (User):", e));
    ...
    // 🚀 التحديث الأهم: إرسال الطلبات كمستندات سحابية بدلاً من مصفوفات
    ...
    // 🚀 تحديث إرسال الإيداعات لتعمل كمستندات سحابية
