   ...
   if (favProds.length === 0) {
            ...
            <div class="th-card ${cardClass} clickable-tx-card" ${clickAction} title="انقر لعرض التفاصيل">
                ...
                <div class="th-body">
                    ...
                    <div class="th-amount-col">
                        <span class="th-order num-en">#${tx.id}</span>
    ...
    renderPayments: function() {
        ...
        myDeposits.forEach(d => {
            ...
            idEl.onclick = (e) => { e.stopPropagation(); UIManager.copyToClipboard(d.id.toString(), idEl); };
        ...
        orders.forEach((o, idx) => {
            ...
            cardElement.innerHTML = `
                ...
                <div class="oh-left">
                    ...
                    <div class="oh-order-box" dir="ltr"><span class="oh-order-number num-en">#${o.id}</span></div>
                </div>`;
    ...
    generatePDFReceipt: async function(config) {
        ...
        const receiptHTML = config.type === 'deposit' ? `
            <div class="receipt-diamond" dir="rtl">
                <div class="header">
                    <div>${brandHTML}</div>
                    <div style="text-align: left;"><div class="doc-title">إيصال إيداع</div><div class="doc-id">#${config.data.id}</div></div>
                </div>
                <div class="body">
                    <div class="info-grid">
        ...
        ` : `
            <div class="receipt-diamond" dir="rtl">
                <div class="header">
                    <div>${brandHTML}</div>
                    <div style="text-align: left;"><div class="doc-title">إيصال بيع منتج</div><div class="doc-id">#${config.data.id}</div></div>
                </div>
                <div class="body">
                    <div class="info-grid">
    ...
    exportReceipt: function(orderId) {
        const o = (LiveStoreData.orders || []).find(x => Number(x.id) === Number(orderId));
        if(!o) return;
        this.generatePDFReceipt({
            type: 'order', filename: `Order_${o.id}.pdf`,
            data: {
                id: o.id, userName: UIManager._getFullName?.(DataManager.user) || 'العميل',
    ...
    exportPaymentReceipt: function(depositId) {
        const d = (LiveStoreData.deposits || []).find(x => Number(x.id) === Number(depositId));
        if(!d) return;
        this.generatePDFReceipt({
            type: 'deposit', filename: `Deposit_${d.id}.pdf`,
            data: {
                id: d.id, userName: DataManager.user?.fullName || 'العميل', method: d.method || '---',
    ...
    renderCountryList: function(countries) {
        ...
        listTarget.innerHTML = active.map(c => `
            <div class="dropdown-item" onclick="ClientSystem.selectCountry('${Utils.escapeHtml(c.nameAr)}', '${c.dialCode}', ${c.phoneLen});">
                <span style="margin-left: 8px;">${c.flagEmoji || '🏳️'}</span>
                <span style="flex: 1;">${Utils.escapeHtml(c.nameAr)}</span>
                <span class="num-en" style="color: var(--text-muted);">${c.dialCode}</span>
            </div>`).join('');
    }
};
