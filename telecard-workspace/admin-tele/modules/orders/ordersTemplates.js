// ============================================================================
// 📦 قوالب الطلبات (modules/orders/ordersTemplates.js) - النسخة الماسية V4.4 💎
// 🎯 الوظيفة: توليد الـ HTML بنظام الهيكلة المرنة (Flexbox) وبدون ترقيعات
// 🚀 التحديث: إصلاح ثغرات تسمية المتغيرات في الفاتورة والتحول لـ O(1) الشامل
// ============================================================================
...
export const OrdersTemplates = {
    ...
    orderInputsCard: (parsedInputs) => {
        ...
        return `<div class="dr-card"><div class="dr-inputs-title"><i class="fa-solid fa-keyboard text-primary"></i> بيانات الطلب</div><div class="dr-inputs-list">${inputsHtml}</div></div>`;
    },
    ...
    financialSnapshotBlock: (snap, status) => {
        ...
        let finalSign = '-', finalColorClass = 'text-warning', finalBgClass = '', finalLabel = '<i class="fa-solid fa-hand-holding-dollar"></i> المخصوم (معلق)';
        
        if (isRefundedOrRejected) {
            finalSign = '+'; finalColorClass = 'text-success'; finalBgClass = 'highlight-success';
            finalLabel = '<i class="fa-solid fa-hand-holding-dollar"></i> إجمالي المسترجع للمحفظة';
        } else if (isCompleted) {
            finalSign = '-'; finalColorClass = 'text-danger'; finalBgClass = 'highlight-danger';
            finalLabel = '<i class="fa-solid fa-hand-holding-dollar"></i> المخصوم من المحفظة';
        }
        
        let discountsHtml = '';
        // 🛡️ تصحيح استخدام المتغيرات داخل منطق الخصومات
        if (snap.offerName && sOfferDisc > 0) discountsHtml += `<div class="fs-11 text-muted mb-5"><i class="fa-solid fa-bolt text-warning"></i> تخفيض عرض (${_esc(snap.offerName)}): <span class="num-en text-danger" dir="ltr">-${RenderHelpers.formatMoney(sOfferDisc, 'USD', 2)}</span></div>`;
        if (snap.couponCode && sCouponDisc > 0) discountsHtml += `<div class="fs-11 text-muted"><i class="fa-solid fa-ticket text-primary"></i> كوبون (${_esc(snap.couponCode)}): <span class="num-en text-danger" dir="ltr">-${RenderHelpers.formatMoney(sCouponDisc, 'USD', 2)}</span></div>`;
        
        const firewallHtml = (snap.isFirewallActive || snap.isFirewallViolated) ? `<div class="mt-10 p-8" style="background: var(--glass-bg); border: 1px dashed var(--danger); border-radius: 6px; font-size: 11px; color: var(--danger);"><i class="fa-solid fa-shield-halved"></i> <b>تنبيه حماية:</b> تم منع البيع بخسارة! تم رفع السعر ليعادل رأس المال.</div>` : '';
        ...
        return `
            <div class="financial-snapshot-box mb-15" style="background: var(--bg-card); border: 1px solid var(--border); border-radius: 8px; padding: 12px;">
                ...
                <div class="dr-receipt-row mt-10">
                    ...
                    <span class="dr-receipt-val num-en fw-bold ${profitClass}" dir="ltr" lang="en">${isCompleted ? profitSign + RenderHelpers.formatMoney(sProfit, 'USD', 2) : '<span class="text-muted fs-11">يُحسب عند الاكتمال</span>'} <span class="fs-11 text-muted">(${_enNum(sMargin, 1)}%)</span></span>
                </div>
                ${firewallHtml}
            </div>`;
    },
    ...
    orderDrawerBody: (data) => {
        ...
        <div class="dr-card dr-client" data-action="view-user" data-id="${_esc(data.userId)}">
            <div class="dr-client-left">${data.avatarHtml}<div>${drawerIdentityHtml}</div></div>
        ...
        <div class="dr-card">
            <div class="dr-prod-header">
                ${data.imgHtml}
                <div class="dr-prod-name">${_esc(data.prodName)} <span class="badge-qty" dir="ltr">x${_esc(data.qty)}</span></div>
            </div>
            ...
            <div class="dr-receipt-box">
                ...
                <div class="dr-receipt-row"><span class="dr-receipt-lbl"><i class="fa-solid fa-box-open text-warning"></i> التكلفة الإجمالية</span><span class="dr-receipt-val num-en text-warning" dir="ltr">${_esc(data.unitCostTxt)}</span></div>
            </div>
            `}

            <div class="dr-receipt-box" style="${data.financialSnapshotHtml ? 'border-top: 1px solid var(--border); padding-top: 15px;' : ''}">
                <div class="dr-receipt-row"><span class="dr-receipt-lbl"><i class="fa-solid fa-circle-info"></i> الحالة</span><span class="oh-status ${data.statusClass}">${_esc(data.sText)}</span></div>
                <div class="dr-receipt-row"><span class="dr-receipt-lbl"><i class="fa-regular fa-clock"></i> توقيت الطلب</span><span class="dr-receipt-val num-en copyable-admin" dir="ltr" data-action="copy-text" data-copy-text="${_esc(data.dateTxt)}">${_esc(data.dateTxt)}</span></div>          
                ${data.durationHtml || ''}
                ${data.fxRateStr ? `<div class="dr-receipt-row"><span class="dr-receipt-lbl"><i class="fa-solid fa-calculator text-warning"></i> سعر الصرف</span><span class="dr-receipt-val num-en text-warning" dir="ltr">${_esc(data.fxRateStr)}</span></div>` : ''}
            </div>
        </div>
        ${data.inputsCardHtml} ${data.codeHtml} ${data.replyHtml}`;
    },

    orderDrawerFooter: (status, orderId) => {
        if (status === 'pending' || status === 'processing') {
            return `<button class="btn btn-green" data-action="submit-order" data-type="accept" data-id="${_esc(orderId)}"><i class="fa-solid fa-check"></i> قبول</button>
                    <button class="btn btn-red" data-action="submit-order" data-type="reject" data-id="${_esc(orderId)}"><i class="fa-solid fa-xmark"></i> رفض</button>`;
        } 
        else if (status === 'completed') {
            return `<button class="btn btn-refund-sky" data-action="request-order-refund" data-id="${_esc(orderId)}"><i class="fa-solid fa-rotate-left"></i> استرجاع وإلغاء الطلب</button>`;
        } 
        else {
            return `<div class="drawer-locked-msg"><i class="fa-solid fa-lock icon-ms-1"></i> هذا الطلب مغلق (${status === 'rejected' ? 'مرفوض' : 'مسترجع'})</div>`;
        }
    },
};