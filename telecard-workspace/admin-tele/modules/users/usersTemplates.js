...
import { DeveloperTemplates } from '../developer/developerTemplates.js';
...
export const UsersTemplates = {
    userSortLabel: (isAsc) => isAsc 
        ? '<span class="sort-icon-group"><i class="fa-solid fa-long-arrow-alt-up"></i><i class="fa-solid fa-long-arrow-alt-down icon-dimmed icon-ms-n2"></i></span>' 
        : '<span class="sort-icon-group"><i class="fa-solid fa-long-arrow-alt-up icon-dimmed icon-me-n2"></i><i class="fa-solid fa-long-arrow-alt-down"></i></span>',
    
    emptyUsers: () => `
        <div class="empty-state">
            <i class="fa-solid fa-users-slash"></i>
            <span>لا يوجد مستخدمون حتى الآن</span>
        </div>
    `,
    ...
    userCard: (u, sortType = 'newest', currentMonthKey = '', lastMonthKey = '', index = 0) => {
        ...
        const kycBadge = u.kycStatus === 'approved' 
            ? `<i class="fa-solid fa-shield-check text-success mx-1" title="حساب موثق (KYC)"></i>` : '';
        ...
        return `
        <div id="user-card-${_esc(u.id)}" class="usr-card ${statusClass}" data-id="${_esc(u.id)}" data-status="${exactStatus}" data-action="view-user">
            ${rankBadgeHtml}
            <div class="usr-top">
                ${avatarHtml}
                <h4 class="usr-name">${name}${kycBadge}</h4>
            </div>
            ${displaySpend}
            <div class="usr-bottom">
                ...
                <div class="usr-wallet">
                    ...
                    <div class="w-amount">
                        <span class="w-val num-en" dir="ltr" lang="en">${RenderHelpers.formatMoney(parseFloat(u.walletBalance ?? u.balance ?? 0), currText, 2)}</span>
                    </div>
                </div>
            </div>
        </div>`;
    ...
    userActivityItem: (tx) => {
        ...
        const classMap = { pending: 'pending', processing: 'processing', completed: 'completed', approved: 'completed', rejected: 'rejected', refunded: 'refunded', returned: 'refunded' };
        ...
        const formatMoneyCompact = (amt, curr) => {
            ...
            const safeDisplayCur = _esc(RenderHelpers.getCurrencySymbolText(curr));
            if (num >= 1e6) {
                const formattedNum = num.toLocaleString('en-US', { notation: 'compact', compactDisplay: 'short', maximumFractionDigits: 2 });
            ...
            } else { return RenderHelpers.formatMoney(amt, curr, 2); }
        };

        return `
        <div class="ud-order-item d-flex align-items-center justify-content-between gap-3 p-3" style="cursor: pointer; transition: background 0.2s; min-height: 70px;" onmouseover="this.style.background='rgba(var(--primary-rgb), 0.05)'" onmouseout="this.style.background='transparent'" data-action="open-tx-detail" data-id="${_esc(tx.id)}" data-type="${_esc(tx.txType)}">
    ...
    userDetailBody: (u, uiData) => {
        const sm = u.isIpBanned 
            ? { t: 'حظر IP', i: 'network-wired', c: 'danger' } 
            : (u.isBanned ? { t: 'محظور', i: 'ban', c: 'danger' } : (u.isRestricted ? { t: 'مقيّد', i: 'lock', c: 'warning' } : { t: 'نشط', i: 'check-circle', c: 'success' }));
        ...
        if (userReview) {
            const stars = '<i class="fa-solid fa-star text-warning"></i>'.repeat(userReview.rating) + 
                          '<i class="fa-regular fa-star text-warning"></i>'.repeat(5 - userReview.rating);
            const mood = userReview.rating >= 4 ? 'عميل راضٍ جداً 🟢' : (userReview.rating <= 2 ? 'عميل غاضب 🔴' : 'عميل محايد 🟡');
            
            ratingHtml = `
            <div class="ud-info-row mt-15 p-10" style="background: rgba(245, 158, 11, 0.05); border: 1px dashed rgba(245, 158, 11, 0.3); border-radius: 8px;">
                <span class="ud-info-lbl text-warning"><i class="fa-solid fa-star"></i> تقييم العميل للمتجر</span>
                <div class="text-start">
                    <div class="mb-5 fs-14">${stars}</div>
                    <span class="fs-11 text-muted">${mood} ${userReview.text ? ` - "${Utils.escapeHTML(userReview.text)}"` : ''}</span>
                </div>
            </div>`;
        } else {
            ratingHtml = `
            <div class="ud-info-row mt-15 p-10" style="background: var(--bg-body); border-radius: 8px;">
                <span class="ud-info-lbl text-muted"><i class="fa-regular fa-star"></i> تقييم العميل للمتجر</span>
                <span class="ud-info-val text-muted fs-11">لم يقم بتقييم المتجر بعد</span>
            </div>`;
        }
        ...
        return `
            <div id="tab-overview" class="ud-tab-content active">
                <div class="ud-section-header ud-header-clean mt-5 mb-10 ud-header-flex">
                    <div class="flex-center-gap">
                        ${u.img ? `<img src="${_esc(u.img)}" class="zoomable-img ud-header-avatar" data-action="open-img-viewer" data-src="${_esc(u.img)}">` : ''}
                        <h3 class="ud-section-title"><i class="fa-solid fa-user-tie"></i> البيانات الشخصية</h3>
                    </div>
            ...
            <div id="tab-orders" class="ud-tab-content">
                <div class="ud-section-header ud-header-clean mt-5 mb-15 ud-header-flex">
                    <h3 class="ud-section-title"><i class="fa-solid fa-receipt"></i> السجل المالي الشامل للعميل</h3>
                </div>
                <div class="sub-tabs-container mb-15 d-flex gap-2 p-5" style="background: var(--glass-bg); border-radius: 8px;">
                    <button class="btn btn-sm btn-primary fh-tab flex-1 active" data-target="fh-orders">
                        <i class="fa-solid fa-box-open"></i> المشتريات
                    </button>
                    <button class="btn btn-sm btn-ghost fh-tab flex-1" data-target="fh-deposits">
                        <i class="fa-solid fa-wallet"></i> الإيداعات
                    </button>
                </div>
                <div id="user-history-list" class="user-history-list mb-15 custom-scrollbar" style="max-height: 400px; overflow-y: auto;">
                </div>
                <div class="text-center">
                    <button class="btn btn-ghost btn-sm" id="btn-load-more-user-history" data-action="load-more-user-history" data-id="${u.id}">
                        <i class="fa-solid fa-angle-down"></i> عرض المزيد (سحابياً)
                    </button>
                </div>
            </div>
            ...
            ${DeveloperTemplates.developerTabContent(u)}
            `;
    },
    
    tierCard: (t, userCount, tStats = { profit: 0, revenue: 0 }) => `
        <div id="tier-card-${_esc(t.id)}" class="tier-card ${t.autoAdvance ? 'auto-on' : 'auto-off'}" data-tier-id="${_esc(t.id)}">
            <div class="tc-head">
                <div class="tc-info">
                    <div class="tc-icon-box"><i class="${_getIcon(t.icon, 'fa-user')}"></i></div>
                    <div class="tc-name">
                        <h2>${_esc(t.name || t.id)}</h2>
                        ${t.isDefault ? '<span class="tc-badge">افتراضي</span>' : ''}
                    </div>
                </div>
                <div class="flex-center-gap">
            ...
            <div class="tc-body">
                ...
                <div class="toggle-box tc-toggle-row">
                    <span class="toggle-lbl">الانتقال التلقائي</span>
                    <label class="switch">
                        <input type="checkbox" ${t.autoAdvance ? 'checked' : ''} data-action="toggle-tier-auto" data-id="${_esc(t.id)}">
            ...
            <div class="tc-foot">
                <div class="count-badge">
                    ...
                    <span class="num-en count-val" dir="ltr" lang="en">${_enNum(userCount)}</span>
                    <span>عميل</span>
                </div>
                <button class="btn btn-ghost tier-users-toggle" data-action="open-tier-users" data-id="${_esc(t.id)}">
                    <i class="fa-solid fa-users"></i> إدارة العملاء
                </button>
            </div>
        </div>`,
        
    tierUsersSectionBody: () => `
        <div class="tier-users-header">
            <button class="btn btn-ghost" data-action="back-to-tiers"><i class="fa-solid fa-arrow-right"></i> العودة للمستويات</button>
            <h2 id="tier-users-title">عملاء المستوى</h2>
        </div>
        <div class="tier-info-card">
            <div class="tic-icon-box"><i class="fa-solid fa-user"></i></div>
            <div class="tic-info"><h3 id="tier-name">المستوى</h3><div class="tier-stats"></div></div>
        </div>
        <div id="tier-users-container" class="tier-users-grid"></div>
    `,
    
    tierInfoStats: (tier, tStats = { profit: 0, revenue: 0 }) => `
        <span class="tier-stat-item"><strong class="stat-lbl-group"><i class="fa-solid fa-sack-dollar stat-icon text-gold"></i> صافي الأرباح:</strong> <span class="num-en text-gold fw-bold" dir="ltr" lang="en">${RenderHelpers.formatMoney(tStats.profit, 'USD', 2)}</span></span>
        <span class="tier-stat-item"><strong class="stat-lbl-group"><i class="fa-solid fa-chart-line stat-icon text-success"></i> نسبة الربح:</strong> <span class="num-en" dir="ltr" lang="en">%${_enNum(tier.profit_percent || 0, 2)}</span></span>
        <span class="tier-stat-item"><strong class="stat-lbl-group"><i class="fa-solid fa-bullseye stat-icon"></i> الشرط المالي:</strong> <span class="num-en" dir="ltr" lang="en">${RenderHelpers.formatMoney(tier.threshold || 0, 'USD', 2)}</span></span>
    `,
    
    tierUsersSearchBox: () => `
        <div class="filter-pro-container mb-15 tier-search-box-wrapper">
            <div class="filter-input-group w-100 mb-0">
                <i class="fa-solid fa-magnifying-glass"></i>
                <input type="text" class="form-input" placeholder="بحث سريع عن عميل..." data-action="filter-tier-users">
            </div>
        </div>
    `,

    emptyTierUsers: () => `
        <div class="empty-state mt-15">
            <i class="fa-solid fa-users"></i>
            <span>لا يوجد عملاء في هذا المستوى</span>
        </div>
    `,
    ...
    kycRequestCard: function(user) {
        ...
        let tiersHtml = `
        <div id="kyc-tiers-selection" class="mt-4 animate__animated animate__fadeIn ${!isSpec ? 'hide-element' : ''}">
            <h6 class="mb-3 fw-bold text-main">تحديد المستويات المستهدفة بالتوثيق:</h6>
            <div class="tiers-selection-list d-grid gap-2">
                 ${tiers.map(t => {
                    ...
                    return `
                    <div class="tier-kyc-item">
                        <div class="d-flex align-items-center gap-3 flex-1 overflow-hidden">
                            <i class="${safeIcon} fs-5" style="color: ${t.color || 'var(--gold)'}; flex-shrink: 0;"></i>
                            <span class="fw-bold text-truncate text-main">${t.nameAr || t.name}</span>
                        </div>
                        <label class="switch m-0 flex-shrink-0">
                            <input type="checkbox" data-action="toggle-kyc-tier" data-id="${t.id}" ${isTargeted ? 'checked' : ''}>
                            <span class="slider"></span>
                        </label>
                    </div>`;
                }).join('')}
        ...
        <div class="kyc-dashboard-card mb-15">
            <div class="kyc-master-switch-box">
                ...
                <div class="kyc-mode-grid">
                    <div class="kyc-mode-btn off-mode ${mode === 'off' ? 'active' : ''}" data-action="update-kyc-mode" data-mode="off">
                        <i class="fa-solid fa-power-off"></i> إيقاف التوثيق
                    </div>
                    <div class="kyc-mode-btn all-mode ${mode === 'all' ? 'active' : ''}" data-action="update-kyc-mode" data-mode="all">
                        <i class="fa-solid fa-globe"></i> فرض على الجميع
                    </div>
                    <div class="kyc-mode-btn spec-mode ${isSpec ? 'active' : ''}" data-action="update-kyc-mode" data-mode="specific">
                        <i class="fa-solid fa-list-check"></i> مستويات محددة
                    </div>
                </div>
            </div>
            ${tiersHtml}
        ...
        <div class="kyc-dashboard-card mb-15" style="border-color: rgba(16, 185, 129, 0.3);">
            <div class="kyc-master-switch-box">
                ...
                <p class="text-muted fs-12 mb-15">تفعيل هذه الخيارات سيمنع العميل من الشراء أو الإيداع حتى يقوم بتفعيلها في جهازه.</p>
                
                <div class="tier-kyc-item mb-10" style="background: var(--bg-body);">
                    <div class="d-flex align-items-center gap-3 flex-1 overflow-hidden">
                        <i class="fa-solid fa-fingerprint fs-4 text-primary" style="flex-shrink: 0;"></i>
                        <div>
                            <div class="fw-bold text-main fs-14">إجبار العملاء على تفعيل البصمة الحيوية</div>
                            <div class="fs-11 text-muted">تقييد عمليات الشراء لمن لم يفعل خيار البصمة</div>
                        </div>
                    </div>
                    <label class="switch m-0 flex-shrink-0">
                        <input type="checkbox" id="policy-force-bio" ${securityPolicy.forceBiometrics ? 'checked' : ''}>
                        <span class="slider"></span>
                    </label>
                </div>
                
                <div class="tier-kyc-item" style="background: var(--bg-body);">
                    <div class="d-flex align-items-center gap-3 flex-1 overflow-hidden">
                        <i class="fa-solid fa-shield-halved fs-4 text-warning" style="flex-shrink: 0;"></i>
                        <div>
                            <div class="fw-bold text-main fs-14">إجبار العملاء على المصادقة الثنائية (2FA)</div>
                            <div class="fs-11 text-muted">تقييد عمليات الشراء لمن لم يفعل المصادقة الثنائية</div>
                        </div>
                    </div>
                    <label class="switch m-0 flex-shrink-0">
                        <input type="checkbox" id="policy-force-2fa" ${securityPolicy.force2FA ? 'checked' : ''}>
                        <span class="slider"></span>
                    </label>
                </div>
            </div>
        ...
        <div class="kyc-save-btn-wrap">
            <button class="btn btn-primary px-5 fw-bold" data-action="save-kyc">
                <i class="fa-solid fa-floppy-disk"></i> حفظ إعدادات الأمان والتوثيق
            </button>
        </div>`; 
    },
    emptyKycState: function(type = 'empty') {
        ...
        return `
        <div class="empty-state mt-15">
            <div class="empty-icon mb-10">
                <i class="fa-solid ${icon} fa-3x ${!isDone ? 'opacity-50' : ''}"></i>
            </div>
            <div class="empty-text fw-bold fs-16 text-main">${title}</div>
            <div class="empty-sub text-muted fs-12 mt-5">${desc}</div>
        </div>`;
    },
    ...
    tierOptionItem: (tier, isCurrent, safeName, safeIcon) => {
        return `
        <div class="tier-option ${isCurrent ? 'current' : ''}" data-tier-id="${tier.id}" data-action="select-tier-option">
            <div class="flex-center-gap-10">
                <div class="tc-icon-box-sm"><i class="${safeIcon}"></i></div>
                <div>
                    <div class="tsm-tier-name">${safeName}</div>
                    <div class="tsm-tier-meta" dir="ltr" lang="en">نسبة الربح: %${_enNum(tier.profit_percent||0, 2)} | شرط: ${RenderHelpers.formatMoney(tier.threshold||0, 'USD', 2)}</div>
                </div>
                ${isCurrent ? '<i class="fa-solid fa-check tsm-check-icon"></i>' : ''}
            </div>
        </div>`;
    },
