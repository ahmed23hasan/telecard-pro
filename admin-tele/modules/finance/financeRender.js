...
export const FinanceRender = {
    ...
    renderDeposits: function(isAppend = false) {
        ...
        if(f.search || f.start || f.end) {
            ...
            data = data.filter(d => {
                let mS = true, mD = true;
                if(f.search) mS = String(d.id).includes(f.search) || String(d.userName).toLowerCase().includes(String(f.search).toLowerCase());
        ...
        let newHtml = paginatedDeposits.map(d => {
            ...
            const fxRate = Number(d.fxRate ?? (convertViaUSD ? convertViaUSD(1, payCurr, targetCurr, AdminData.data.rates, 'deposit') : 1));
    ...
    exportDepositsToExcel: function() {
        ...
        let csvContent = "\uFEFFرقم الإيداع,التاريخ,العميل,البنك/الطريقة,المبلغ,العملة,الحالة\n";
        dataToExport.forEach(d => {
            const dateStr = Utils.formatDate(d.time);
            const user = String(d.userName || d.userId).replace(/,/g, " ");
            const method = String(d.method || d.methodName || '').replace(/,/g, " ");
            const amount = Number(d.amount || 0).toFixed(2);
            const curr = String(d.currency || 'USD').toUpperCase();
            const status = d.status === 'approved' ? 'مقبول' : (d.status === 'rejected' ? 'مرفوض' : d.status);
            csvContent += `${d.id},${dateStr},${user},${method},${amount},${curr},${status}\n`;
        });
