...
export const DashboardTemplates = {
    ...
    // 🌟 واجهة تحليل العروض والخصومات الجبارة (بشكل الكبسولات القياسية الموحدة والعبارات الاحترافية)
    dashCouponsSection: (promoStats) => `
        ...
        <div class="dash-circ-grid">
            <div class="dash-circ-card" data-action="nav" data-target="coupons">
                <div class="circ-icon bg-danger"><i class="fa-solid fa-percent"></i></div>
                <div class="circ-data">
                    <h3 class="num-en text-danger" dir="ltr" lang="en">${RenderHelpers.formatMoney(promoStats.totalDiscountAmount, 'USD', 2)}</h3>
            ...
            <div class="dash-circ-card" data-action="nav" data-target="coupons">
                ...
                <div class="circ-data">
                    <h3 class="num-en text-gold" dir="ltr" lang="en">${RenderHelpers.formatMoney(promoStats.discountedRevenue, 'USD', 2)}</h3>
                    <span>عائدات الخصومات</span>
                </div>
            </div>
            
            <div class="dash-circ-card" data-action="nav" data-target="coupons">
                <div class="circ-icon bg-success"><i class="fa-solid fa-cart-arrow-down"></i></div>
                <div class="circ-data">
                    <h3 class="num-en text-success" dir="ltr" lang="en">${_enNum(promoStats.totalDiscountedOrders)}</h3>
                    <span>الطلبات المخفضة</span>
                </div>
            </div>
            
            <div class="dash-circ-card" data-action="nav" data-target="coupons">
                <div class="circ-icon bg-purple"><i class="fa-solid fa-ticket"></i></div>
                <div class="circ-data">
                    <h3 class="num-en text-purple" dir="ltr">${_esc(promoStats.topCoupon)}</h3>
                    <span>الكوبون الأكثر استخداماً</span>
                </div>
            </div>
            
            <div class="dash-circ-card" data-action="nav" data-target="coupons">
                <div class="circ-icon bg-info"><i class="fa-solid fa-bolt"></i></div>
                <div class="circ-data">
                    <h3 class="num-en text-info" dir="ltr">${_esc(promoStats.topOffer)}</h3>
                    <span>حملة التخفيض الأنجح</span>
                </div>
            </div>
        </div>
    `,
    ...
    dashPodium: (topUsers) => {
        ...
        ranks.forEach(item => { if (item.user) { html += `<div class="podium-item ${item.rankClass} clickable" data-action="view-user" data-id="${_esc(item.user.id)}"><div class="podium-avatar-wrap"><div class="podium-rank-num">${item.num}</div><div class="rank-badge">${item.badge}</div></div><div class="podium-info"><span class="uid-capsule" title="رقم العميل"><i class="fa-solid fa-hashtag"></i>${_esc(item.user.id)}</span><span class="podium-value text-gold num-en" dir="ltr">${RenderHelpers.formatMoney(item.user.spent, 'USD', 2)}</span></div></div>`; } });
        return html + '</div></div>';
    },
    ...
    dashActiveUserCapsule: (user) => {
        ...
        return `<div class="dash-circ-card highlight-active clickable" data-action="view-user" data-id="${_esc(user.id)}"><div class="circ-icon bg-warning">${avatar}</div><div class="circ-data"><span>العميل الأكثر نشاطاً</span><div class="mt-15"><span class="uid-capsule" title="رقم العميل"><i class="fa-solid fa-hashtag"></i>${_esc(user.id)}</span></div></div></div>`;
    },
