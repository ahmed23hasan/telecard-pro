...
export const SalesRender = {
    ...
    renderSales: function() {
        ...
        if (catsTbody) {
            ...
            if (sortedCats.length === 0) {
                ...
                catsTbody.innerHTML = sortedCats.map(c => {
                    ...
                    return `
                        <tr>
                            <td class="fw-bold">${Utils.escapeHTML(c.name)}</td>
                ...
                const buildRankHtml = (prod, rankClass, num, badgeIcon) => {
                    ...
                    <div class="podium-item ${rankClass}">
                        ...
                        <div class="podium-info">
                            <span class="podium-name" title="${Utils.escapeHTML(prod.name)}">${Utils.escapeHTML(prod.name)}</span>
        ...
        if (allProdsTbody) {
            if (sortedProds.length === 0) {
                // تعديل الدمج لـ 5 أعمدة بدلاً من 3
                ...
                allProdsTbody.innerHTML = sortedProds.map(p => {
                    ...
                    return `
                        <tr>
                            <td class="fs-12 fw-bold">${Utils.escapeHTML(p.name)}</td>
    ...
    exportSalesToExcel: function() {
        ...
        Object.values(stats.products).forEach(p => {
            csv += `"${p.name}",${p.count},${p.revenue.toFixed(2)},${p.cost.toFixed(2)},${p.profit.toFixed(2)}\n`;
        });
