...
export const DashboardRender = {
    ...
    renderDashboard: function() {
        ...
        const buildAlertHtml = (a) => {
            ...
            if (a.id === 'vault_empty') { type = 'danger'; icon = 'fa-box-open'; text = AdminTemplates.alertVaultEmpty(RenderHelpers._esc(a.poolName)); action = `AdminApp.openVaultModal('${a.poolId}')`; } 
            else if (a.id === 'vault_low') { type = 'warning'; icon = 'fa-hourglass-half'; text = AdminTemplates.alertVaultLow(RenderHelpers._esc(a.poolName), a.count); action = `AdminApp.openVaultModal('${a.poolId}')`; } 
            else if (a.id === 'coupon_used') { type = 'success'; icon = 'fa-tag'; text = AdminTemplates.alertCouponUsed(RenderHelpers._esc(a.user), RenderHelpers._esc(a.code)); action = `AdminUI.openOrderDrawer('${a.orderId}')`; } 
            else if (a.id === 'offer_expiring') { type = 'warning'; icon = 'fa-bolt'; text = AdminTemplates.alertOfferExpiring(RenderHelpers._esc(a.name)); action = "AdminApp.nav('coupons', document.querySelector('.nav-item[data-target=\\'coupons\\']'))"; } 
            else if (a.id === 'security_stable') { type = 'security'; icon = 'fa-shield-halved'; text = AdminTemplates.alertSecurityStable(); }
