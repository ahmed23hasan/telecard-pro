...
export const OrdersRender = {
        ...
        if(f.search || f.start || f.end) {
            ...
            data = data.filter(o => {
                ...
                if(f.search) {
                    ...
                    mS = String(o.id).includes(s) || (o.product && String(o.product).toLowerCase().includes(s)) || (o.userName && String(o.userName).toLowerCase().includes(s));
                }
    ...
    exportToExcel: function() {
        ...
        let csvContent = "\uFEFFرقم الطلب,التاريخ,العميل,المنتج,الكمية,السعر الاجمالي($),التكلفة($),الربح($),المصدر,الحالة\n";
        dataToExport.forEach(o => {
            ...
            const customer = sanitizeCSV(o.userName || o.userId); 
            ...
            const source = (o.isApi || o.source === 'api') ? 'API' : 'Manual';
            const status = o.status === 'completed' ? 'مكتمل' : (o.status === 'rejected' ? 'مرفوض' : (o.status === 'refunded' ? 'مسترجع' : o.status));

            csvContent += `${o.id},${dateStr},${customer},${product},${qty},${exactPrice.toFixed(2)},${totalCost.toFixed(2)},${profit.toFixed(2)},${source},${status}\n`;
        });
