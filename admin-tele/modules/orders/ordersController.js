...
// الوظيفة: معالجة العمليات المنطقية والمالية المعقدة للطلبات (قبول، رفض، استرجاع)
// ============================================================================
...
export const OrdersController = {
  ...
  submitOrderAction: async function(action, orderId) {
    ...
    if (action === 'accept') {
      ...
      // خصم الإحصائيات إذا كان استرجاعاً لطلب مكتمل
      if (AdminData.data.system && AdminData.data.system.globalStats) {
        const gs = AdminData.data.system.globalStats;
      ...
      // تعليم أكواد الخزنة كتالفة (Defective) لمنع إعادة بيعها
      if (o.deliveredCode) {
    ...
    // الحفظ المجمع للبيانات لتقليل عمليات الكتابة (Writes Optimization)
