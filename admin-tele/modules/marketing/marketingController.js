...
export const MarketingController = {
    ...
    sendUnifiedAlert: async function() {
        ...
        if (targetType === 'tier') {
            ...
            targetId = Utils.escapeHTML(Utils.getVal('alert-target-user'));
            if (!targetId) {
                EventBus.emit('req-show-toast', { message: 'يرجى إدخال رقم العميل (ID)', type: 'error' });
                return;
            }
            const userExists = (AdminData.data.users || []).find(u => String(u.id) === String(targetId));
            if (!userExists) {
