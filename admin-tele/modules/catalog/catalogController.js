...
export const CatalogController = {
    ...
    saveCountry: async function() {
        ...
        const newCountry = { id: editId || code, nameAr, flagEmoji, dialCode, phoneLen, isActive, isBanned };
    ...
    deleteCountry: async function(id) {
        const country = AdminData.data.countries.find(c => String(c.id) === String(id));
        AdminData.data.countries = AdminData.data.countries.filter(c => String(c.id) !== String(id));
        await AdminData?.saveCountries?.();
        if (country) AdminData?.addLog?.('DELETE_COUNTRY', `تم حذف دولة: ${country.nameAr}`);
