...
export const CatalogTemplates = {
    ...
    countryCard: (c) => {
        const status = c.isBanned ? 'banned' : (c.isActive ? 'active' : 'inactive');
        return `
        <div class="card item-card country-card-item country-${status}" data-id="${_esc(c.id)}">
            <div class="country-header">
                <div class="country-info-group">
                    <span class="country-flag-lg">${c.flagEmoji || '🏳️'}</span>
                    <div class="country-info-meta">
                        <div class="flex-center-gap">
                            <span class="country-name-lg fw-bold">${_esc(c.nameAr)}</span>
                            ${c.isBanned ? '<span class="status-badge rejected fs-9">محظورة</span>' : (!c.isActive ? '<span class="status-badge pending fs-9">معطلة</span>' : '')}
                        </div>
                        <div class="country-meta-text">
                            <span class="num-en text-info" dir="ltr">${_esc(c.dialCode)}</span>
                            <span class="meta-sep">|</span>
                            <span class="num-en">ISO: ${_esc(c.id)}</span>
                        </div>
                    </div>
                </div>
