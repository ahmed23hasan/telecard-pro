...
export const AppController = {
    ...
    init: async function() {
        ...
        try {
            ...
            window.addEventListener('storage', async () => {
                // مزامنة البيانات إذا تم تعديلها من تبويب آخر
                await AdminData?.loadData?.();
                
                // 🌟 التأمين الثاني: تحديث المحرك أيضاً عند المزامنة
                RenderHelpers?.init?.(AdminData.data); 
        ...
        } catch (error) { console.error("🚨 خطأ إقلاع:", error); } 
        finally { AdminUI?.toggleLoader?.(false); }
    },
    ...
    logoutAdmin: function() { sessionStorage.removeItem('telecard_admin_auth'); window.location.reload(); },
    ...
    setupEventBusListeners: function() {
        ...
        // 🌟 [الإصلاح الجذري] التقاط أحداث النوافذ والمعرفات المؤقتة التي كانت مفقودة
        ...
        // التقاط حدث تحديد المستوى للعميل
