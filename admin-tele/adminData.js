...
import { FirebaseAdapter } from './core/firebaseAdapter.js'; // 👈 استيراد محول السحابة الجديد
...
export const AdminData = {
    ...
    loadData: async function() {
        ...
        const rawTiers = await fetchArray(DB_KEYS.TIERS, []);
        this.data.tiers = arr(rawTiers).map(t => {
            ...
            return { ...t, threshold, autoAdvance };
        });
        ...
        const rawUsers = await fetchArray(DB_KEYS.USERS, []);
        ...
        this.data.deposits = arr(await fetchArray(DB_KEYS.DEPOSITS, []));
        this.data.orders = arr(await fetchArray(DB_KEYS.ORDERS, []));
        this.data.cats = arr(await fetchArray(DB_KEYS.CATS, []));
        this.data.prods = arr(await fetchArray(DB_KEYS.PRODS, []));
        
        const rawPayments = await fetchArray(DB_KEYS.PAYMENTS, []);
        this.data.payments = arr(rawPayments).map(p => { return { ...p, currencies: normalizeCurrencyList(p.currencies).join(',') }; });
        
        this.data.banners = arr(await fetchArray(DB_KEYS.BANNERS, []));
        this.data.settings = obj(await fetchSingleton(DB_KEYS.SETTINGS, { promoText: "", sliderDuration: 3, sliderTransition: "fade", promoAnim: "vertical-normal", supportLink: "", supportIcon: "fa-whatsapp", supportAnimation: "none" }));
        this.data.rates = normalizeRates(rawRates);
        this.data.notif = obj(await fetchSingleton(DB_KEYS.POPUP, {}));
        this.data.system = obj(await fetchSingleton(DB_KEYS.SYSTEM, { maint: false }));
        this.data.adminProfile = obj(await fetchSingleton(DB_KEYS.ADMIN, { name: "المدير العام", email: "admin@telecard.pro", pass: "123", img: "https://cdn-icons-png.flaticon.com/512/3135/3135715.png" }));

        this.data.countries = arr(await fetchArray(DB_KEYS.COUNTRIES, []));
        if(this.data.countries.length === 0) await this.seedDefaultCountries();

        this.data.vault = arr(await fetchArray(DB_KEYS.VAULT, []));
        this.data.coupons = arr(await fetchArray(DB_KEYS.COUPONS, []));
        this.data.offers = arr(await fetchArray(DB_KEYS.OFFERS, []));
        this.data.logs = arr(await fetchArray(DB_KEYS.LOGS, []));
        this.data.alerts = arr(await fetchArray(DB_KEYS.ALERTS, []));
        this.data.kyc = arr(await fetchArray(DB_KEYS.KYC || 'telecard_kyc', [])); 
        ...
        console.log("✅ اكتملت المزامنة بنجاح وبأقل تكلفة قراءة.");
        return true; 
    },
    ...
    saveCollection: async function(key, prop) {
        try {
            ...
            // 1. إذا كان كائن منفرد (إعدادات، بروفايل) يحفظ كـ Singleton Document
            if (!isArray) {
            ...
            // 2. إذا كان Array (الطلبات، المستخدمين، إلخ)، نقوم بتشغيل محرك الـ Diffing
            const currentArr = currentData || [];
            const snapArr = this._snapshots[prop] || [];

            // ضمان وجود ID لكل عنصر لتجنب الأخطاء
            ...
            // أ. اكتشاف التعديلات والإضافات الجديدة
            ...
            // إرسال التحديثات الضرورية فقط للسحابة! 🚀
    ...
    saveKyc: async function() { return await this.saveCollection(DB_KEYS.KYC || 'telecard_kyc', 'kyc'); }, 
    ...
    // 🚀 تحديث جذري: التصدير والاستيراد المباشر من الذاكرة الحية
    ...
    importData: function(input) {
        return new Promise((resolve, reject) => {
            ...
            const propToKey = {
                ...
                logs: DB_KEYS.LOGS, alerts: DB_KEYS.ALERTS, kyc: DB_KEYS.KYC || 'telecard_kyc', 
            ...
            reader.onload = async (e) => {
                try {
                    ...
                    if(importedData && typeof importedData === 'object') {
                        // إرسال البيانات المجمعة إلى السحابة تلقائياً عبر المحرك
    ...
    seedDefaultTiers: async function() { this.data.tiers = []; await this.saveTiers(); },
    seedDefaultCountries: async function() { this.data.countries = []; await this.saveCountries(); },
    ...
    // ==========================================
    // 🌟 محرك الفلترة الزمنية الذكي (Smart BI Engine)
    // ==========================================
    ...
    autoAdvanceSweep: async function() {
        ...
        // 🚀 بفضل الـ Smart Diffing، دالة الحفظ سترسل فقط المستخدم الذي تغيرت بياناته!
    ...
    getDashboardStats: function(leaderboardPeriod = 'all') {
        ...
        let leaderboard = (d.users || []).map(u => ({ id: u.id, name: u.username ? `@${u.username}` : (u.fullName || 'مستخدم جديد'), img: u.img || null, spent: leaderboardPeriod === 'all' ? (Number(u.totalSpent) || 0) : (Number(u.monthlySpent?.[lbKey]) || 0), count: leaderboardPeriod === 'all' ? (Number(u.totalOrdersCount) || 0) : (Number(u.monthlyOrders?.[lbKey]) || 0) }));
