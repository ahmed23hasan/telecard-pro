...
// ============================================================================
// 🧮 دالة الصيانة: تصحيح أرقام محفظة العميل (Financial Audit Tool)
// ============================================================================
exports.adminAuditUserWallet = functions.region('us-east1').https.onCall(async (data, context) => {
    ...
    try {
        ...
        let realTotalDeposit = 0;
        
        // 1. حساب كل الطلبات المكتملة
        const ordersSnap = await db.collection('telecard_orders')
            .where('userId', '==', targetUserId)
            .where('status', '==', 'completed')
            .get();
        
        ordersSnap.forEach(doc => {
            realTotalSpent += Number(doc.data().price || 0);
        });
        
        // 2. حساب كل الإيداعات وعمليات الإضافة/الخصم من الإدارة
        const depositsSnap = await db.collection('telecard_deposits')
            .where('userId', '==', targetUserId)
        ...
        depositsSnap.forEach(doc => {
            ...
            if (amount > 0) {
                realTotalDeposit += amount; // إيداع أو إضافة من الإدارة
            } else {
                realTotalSpent += Math.abs(amount); // خصم من الإدارة
            }
        });
        
        // 3. الرصيد المفترض = الوارد - الصادر
        const expectedBalance = Math.max(0, realTotalDeposit - realTotalSpent);
        
        // 4. تحديث حساب العميل بالأرقام الحقيقية الموثقة بالفواتير
        await userRef.update({
            totalSpent: Number(realTotalSpent.toFixed(4)),
            totalDeposit: Number(realTotalDeposit.toFixed(4)),
            walletBalance: Number(expectedBalance.toFixed(4)),
            balance: Number(expectedBalance.toFixed(4))
        });
        
        return {
            success: true,
            message: 'تم تصحيح ومطابقة حسابات العميل بنجاح!',
            data: { spent: realTotalSpent, deposit: realTotalDeposit, balance: expectedBalance }
        };
